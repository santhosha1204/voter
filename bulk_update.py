...
# put your file name here should be an .CSV file and the voter ID columns should be names as "ID"
file_name = ''
df_ = pd.read_csv(file_name)
voter_ids = df_['ID'].to_list()
final = []
for voter_id in voter_ids[0:1]:
    print(voter_id)
    for i in range(5):
        data = process(voter_id)
        if data['status'] == 200:
            final.append(data)
    else:
        final.append({'status': 400, 'reason': 'Sorry server down!'})

df = pd.DataFrame(final)
df.to_csv(f'output_{file_name}', index=False)
